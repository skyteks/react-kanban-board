...
function PinnedNote({ noteTextArray }) {
    ...
    const randomColor = colors.at(Math.floor(Math.random() * colors.length));

    const maxAngle = 15;
    const randomAngle = Math.random() * maxAngle - maxAngle * 0.5;

    return (
        <div style={{ width: "400px", height: "400px", backgroundColor: "rgba(0,0,0,0)", display: "flex", justifyContent: "center", alignItems: "center", }}>
            <div style={{ width: "80%", height: "80%", backgroundColor: "rgba(0,0,0,0)", rotate: randomAngle + "deg", display: "flex", alignItems: "center", flexDirection: "column", boxShadow: "rgba(0,0,0,0.5) -8px 20px 9px" }}>
                <img src={pin} style={{ width: "50px", height: "50px", alignSelf: "center", marginLeft: "25px", position: "absolute" }} />
                <div style={{ width: "100%", height: "100%", backgroundColor: randomColor }}>
